...
router.post('/login', async (req, res) => {
    ...
    if (error) res.status(201).send(error.details[0].message);

    // Check if email already exists
    const user = await User.findOne({ email: req.body.email });
    if (!user) return res.status(201).send('Wrong username or password');
    
    // Compare passwords
    const match = await comparePasswords(req.body.password, user.password);
    if (!match) return res.status(201).send('Wrong username or password');
